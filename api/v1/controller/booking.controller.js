    ...
    create: async (req, res) => {
        try {
            const result = await baseModel.executeTransaction(async () => {
                ...
                const newDetails = await Promise.all(req.body.serviceID.map(async (serviceID) => {
                    req.body.serviceID = serviceID;  // Cập nhật serviceID hiện tại
                    const { columns: columnsDetail, values: valuesDetail } = getColsVals(detailTable, req.body);
                    return await queryModel.create(detailTable.name, columnsDetail, valuesDetail);
                }));

                const { columns: columnsStylistWorkShift, values: valuesStylistWorkShift } = getColsVals(stylistWorkShiftTable, req.body);
                const updateWorkshift = await baseModel.update(stylistWorkShiftTable.name, stylistWorkShiftTable.columns.stylistID, newBooking.stylistID, columnsStylistWorkShift, valuesStylistWorkShift)
